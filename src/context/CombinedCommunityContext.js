...
export const CombinedCommunityProvider = ({ children }) => {
    ...
    const loadCommunitiesData = useCallback(async (sub_community_id) => {
        setLoading(true);
        setLoadingMainCommunity(true);
        setLoadingSubCommunity(true);
        try {
            const tempSubCommunityData = await getSubCommunityById(sub_community_id);
            if (tempSubCommunityData) {
                setSubCommunityData(tempSubCommunityData);
                const tempMainCommunityData = await getCommunityById(tempSubCommunityData.community_id);
